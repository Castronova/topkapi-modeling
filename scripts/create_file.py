...
def generate_param_file(ini_fname, isolated_cells=False):
    ...
    # if strahler raster is given use it, otherwise calculate strahler
    strahler_fname = None
    if config.has_option('raster_files', 'strahler_fname'):
    ...
    # we are using an ArcGIS raster that is converted into a tiff in which nodata=-9999, non-streams=0, streams=1
    channel_network[channel_network < 1] = 0  # set all non-channel elements to 0
    ...
    if isolated_cells == True:
        ...
        # calculate mannings roughness from strahler stream order
        if strahler_fname is None:
            ...
            n_c = strahler_to_channel_manning(cell_labels,
                                              channel_network[mask == 1],
                                              cell_down)
        # read either strahler or mannings roughnes from input file (you choose prasanna)
        else:
            # todo: prasanna insert code here to derive or read mannings numbers from strahler input file.
